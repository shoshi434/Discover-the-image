 ...
 const israelImage=document.querySelectorAll(".israelImage")
...
const usersArr=JSON.parse(localStorage.getItem("userArr"))//מקבל את כל מערך המשתמשים מהלוקל סטורג
 ...
 let index =currentUser.index1+1// אינדקס של מערך השאלות
 ...
 const changeImg=()=>{//פונקציה שמחליפה תמונה 
    let randomPicture=Math.floor(Math.random()*israelImage.length)
    let selectedImage = israelImage[randomPicture].src
